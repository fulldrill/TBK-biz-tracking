...
from app.models import User, BankAccount, OrgRole
...
router = APIRouter(prefix="/orgs/{org_id}/bank", tags=["Bank"])
logger = logging.getLogger(__name__)

# --- Legacy aliases (backward compat) ---
legacy_router = APIRouter(prefix="/bank", tags=["Bank (legacy)"])
...
# --- Legacy routes: infer org from user's first org ---

@legacy_router.get("/link-token", response_model=LinkTokenResponse)
async def legacy_get_link_token(current_user: User = Depends(get_current_user)):
    try:
        token = await create_link_token(str(current_user.id))
        return LinkTokenResponse(link_token=token)
    except Exception as e:
...
@legacy_router.post("/connect")
async def legacy_connect_bank(
    payload: ExchangeTokenRequest,

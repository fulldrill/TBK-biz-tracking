...
from app.models import User, Transaction, TransactionType, OrgRole
from app.schemas import TotalsResponse
from app.auth import get_current_user, require_org_role
from datetime import datetime, timedelta
from typing import Optional

router = APIRouter(prefix="/orgs/{org_id}/totals", tags=["Totals"])
legacy_router = APIRouter(prefix="/totals", tags=["Totals (legacy)"])
...
@legacy_router.get("/", response_model=TotalsResponse)
async def legacy_get_totals(
    start_date: Optional[datetime] = None,
    end_date: Optional[datetime] = None,
...
@legacy_router.get("/monthly")
async def legacy_get_monthly_breakdown(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(

...
def _ensure_dir(output_path: str) -> None:
    """Create the parent directory, tolerating a bare filename.

    os.path.dirname("receipt.pdf") is "", and os.makedirs("") raises.
    """
    directory = os.path.dirname(output_path)
    if directory:
        os.makedirs(directory, exist_ok=True)

def _receipt_flowables(transaction_data: dict, org_name: str, styles) -> list:
    """The body of one receipt, reusable for both single and combined output."""
    elements = []

    title_style = ParagraphStyle(
        "ReceiptTitle", parent=styles["Title"], fontSize=18,
        textColor=colors.HexColor("#1a1a2e"), spaceAfter=2, alignment=0,
    )
    elements.append(Paragraph(org_name or "Clerq", title_style))
    elements.append(Paragraph("Transaction Receipt", styles["Heading3"]))
    ...
    if transaction_data.get("assigned_user"):
        data.append(["Attributed to", str(transaction_data["assigned_user"])])

    # Provenance. Without these a receipt is a standalone summary; with them it
    # points at the primary document, which is what substantiation requires.
    if transaction_data.get("account_label"):
        data.append(["Account", str(transaction_data["account_label"])])
    if transaction_data.get("statement_period"):
        data.append(["Statement period ending", str(transaction_data["statement_period"])])
    if transaction_data.get("statement_file"):
        data.append(["Source document", str(transaction_data["statement_file"])])
    ...
    if transaction_data.get("statement_file"):
        provenance = (
            "<i>This entry is transcribed from the account statement named above, issued by the "
            "financial institution. Retain that statement as the primary record; this receipt is "
            "a supporting voucher for bookkeeping.</i>"
        )
    else:
        provenance = (
            "<i>This entry is derived from the account's transaction history. Retain the "
            "corresponding bank statement as the primary record; this receipt is a supporting "
            "voucher for bookkeeping.</i>"
        )
    elements.append(Paragraph(provenance, styles["Normal"]))
...
def generate_receipt_pdf(transaction_data: dict, output_path: str, org_name: str = "") -> str:
    ...
    doc.build(_receipt_flowables(transaction_data, org_name, styles))
    return output_path


def generate_all_receipts_pdf(
    transactions: List[dict], output_path: str, org_name: str = "", period_label: str = ""
) -> str:
    """One receipt per page, in date order — the whole set as a single document."""
    _ensure_dir(output_path)
    doc = SimpleDocTemplate(
        output_path, pagesize=letter,
        leftMargin=0.75 * inch, rightMargin=0.75 * inch,
    ...
    # Cover page: what this bundle is, so a stack of receipts is self-describing.
    elements.append(Paragraph(org_name or "Clerq", ParagraphStyle(
        "CoverTitle", parent=styles["Title"], fontSize=22,
        textColor=colors.HexColor("#1a1a2e"), spaceAfter=4,
    )))
    elements.append(Paragraph("Transaction Receipts", styles["Heading2"]))
    if period_label:
        elements.append(Paragraph(period_label, styles["Normal"]))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a1a2e")))
    elements.append(Spacer(1, 0.25 * inch))

    total = sum(abs(t.get("amount", 0) or 0) for t in transactions)
    credits = sum(abs(t.get("amount", 0) or 0) for t in transactions
                  if str(t.get("transaction_type")) == "credit")
    summary = Table(
        [
            ["Receipts in this document", str(len(transactions))],
            ["Total money in", _money(credits)],
            ["Total money out", _money(total - credits)],
        ],
    ...
    elements.append(Paragraph(
        "<i>Each following page is one transaction receipt, in date order.</i>",
        styles["Normal"],
    ))

    for tx in transactions:
        elements.append(PageBreak())
        elements.extend(_receipt_flowables(tx, org_name, styles))

    doc.build(elements)
    return output_path

def generate_batch_receipt_pdf(transactions: List[dict], totals: dict, output_path: str, period_label: str = "", org_name: str = "") -> str:
    _ensure_dir(output_path)
    doc = SimpleDocTemplate(output_path, pagesize=letter, leftMargin=0.75*inch, rightMargin=0.75*inch)
    styles = getSampleStyleSheet()
    elements = []

    elements.append(Paragraph(org_name or "Clerq", styles["Title"]))
